...
| **Blocking** | Local and weak by design: it declines to display something that was delivered and stored, it does not prevent delivery. `contactPolicy` is named as the server-side half that would, and is **not built** — the field exists, is persisted, and enforces nothing. |
